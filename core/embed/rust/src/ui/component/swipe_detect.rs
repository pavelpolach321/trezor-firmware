...
impl SwipeConfig {
    ...
    pub fn configure_paging(&mut self, current_page: u16, total_pages: u16) {
    ...
    pub fn paging_event(&self, dir: Direction) -> i8 {
        if !self.is_allowed(dir) {
            return 0;
        }
        match self.page_axis {
            Some(Axis::Horizontal) => match dir {
                Direction::Right => -1,
                Direction::Left => 1,
                _ => 0,
            },
            Some(Axis::Vertical) => match dir {
                Direction::Down => -1,
                Direction::Up => 1,
                _ => 0,
            },
                ...
                if animation.finished(Instant::now()) {
                    ...
                    if animation_disabled() {
                        ...
                        Some(SwipeEvent::Move(
                            locked,
                            animation.value(Instant::now()).max(0) as i16,
                        ))
                    }
                }
            } else {
